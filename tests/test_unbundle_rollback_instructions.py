...
class TestRollbackInstructionsAndTimestamps(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(
            [sys.executable, "-m", "dwimsy.meta.bundle", "-o", str(cls.bundle_path), "--with-deps"],
            cwd=str(src_dir),
            check=True,
        )
    ...
    def test_version_list_timestamps(self):
        ...
        self.assertGreater(len(timestamps), 3, "Expected multiple distinct timestamps across layers")
        self.assertTrue(any("2026-09-06" in ts for ts in timestamps))
        self.assertTrue(any("2026-08-29" in ts or "2026-08-30" in ts or "2026-08-31" in ts for ts in timestamps))
    ...
    def test_upgrade_prints_rollback_instructions(self):
        ...
        subprocess.run(
            [sys.executable, str(self.bundle_path), "--version=0.1.6.70-dev", "meta", "unbundle", str(target_dir), "--deps", "--force", "--quiet"],
            check=True,
        )
        # Upgrade to latest
        res = subprocess.run(
            [sys.executable, str(self.bundle_path), "meta", "unbundle", str(target_dir), "--deps"],
            capture_output=True,
            text=True,
            check=True,
        )
        self.assertIn("To roll back to previous version '0.1.6.70-dev', run: python3", res.stdout)
    ...
    def test_destructive_rollback_requires_force_or_alt(self):
        ...
        # Setup clean head
        subprocess.run(
            [sys.executable, str(self.bundle_path), "meta", "unbundle", str(target_dir), "--deps", "--force", "--quiet"],
            check=True,
        )
        # Unsafe rollback without --force or --version-alt should fail
        res = subprocess.run(
            [sys.executable, str(self.bundle_path), "--version=0.1.6.70-dev", "meta", "unbundle", str(target_dir), "--deps"],
            capture_output=True,
            text=True,
        )
        ...
        res_forced = subprocess.run(
            [sys.executable, str(self.bundle_path), "--version=0.1.6.70-dev", "--force", "meta", "unbundle", str(target_dir), "--deps"],
            capture_output=True,
            text=True,
            check=True,
        )
        ...
        subprocess.run(
            [sys.executable, str(self.bundle_path), "meta", "unbundle", str(target_dir_alt), "--deps", "--force", "--quiet"],
            check=True,
        )
        res_alt = subprocess.run(
            [sys.executable, str(self.bundle_path), "--version-alt", "--version=0.1.6.70-dev", "meta", "unbundle", str(target_dir_alt), "--deps"],
            capture_output=True,
            text=True,
            check=True,
        )
