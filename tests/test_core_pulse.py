...
@unittest.skipUnless(
    ...
    "aren't available in this environment (they're a personal, "
    "non-redistributable development loan — see the README's Test "
    "Fixtures section — so this equivalence check only runs where "
    "they happen to already be present locally).",
)
class TestPulseEquivalenceAgainstOriginal(unittest.TestCase):
    ...
    def setUp(self):
        self.orig = _load_module("orig_wav2t88_for_pulse_check", str(ORIGINAL_TOOL_PATH))
                ...
                for s in samples:
                    if use_new_frontend:
                        ...
                        if ev.kind == "silence":
                            ...
                            sym = classify(
                                ev.period_sec, ev.envelope, ev.peak_carrier,
                                ev.noise_floor,
                            )
